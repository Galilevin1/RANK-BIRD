"""
Figure 2E optional: Jaccard similarity of SHAP features vs LODO performance,
aggregated across all phenotypes.

This is the cross-phenotype aggregation of panel 2E — instead of one phenotype,
it pools all pairs from all phenotypes to ask the global question:
"does feature similarity predict cross-dataset generalisation?"

Requires pairwise LODO + SHAP to have been computed first (via run_pairwise_lodo_analysis
or via the 2E cache in figures_out/figure_2/2e/).

Produces 4 plots:
  overall_scatter.png      — all pairs, colored by phenotype
  per_phenotype_grid.png   — one scatter panel per phenotype
  distributions.png        — histograms of Jaccard and LODO AUC
  correlations.png         — Pearson r / Spearman ρ per phenotype
"""
...
def run_figure2e_optional(
    pairwise_lodo_dir: str,
    metric: str = "auc",
) -> Optional[Dict]:
    """
    Load pairwise LODO + SHAP results for all phenotype subfolders,
    compute Jaccard similarity between SHAP feature sets, correlate with
    pairwise LODO performance, and return aggregated statistics.

    pairwise_lodo_dir : directory produced by run_pairwise_lodo_analysis
                        (contains one subfolder per phenotype)

    Returns None if no usable phenotype data is found.
    """
    base = Path(pairwise_lodo_dir)
    ...
    if not phenotype_dirs:
        print(f"No pairwise LODO results found in {pairwise_lodo_dir}. "
              f"Run pairwise LODO analysis first (run key '4' or '2').")
        return None

    print(f"\n2E optional: found {len(phenotype_dirs)} phenotypes")
    ...
    if len(all_jaccard) < 3:
        print("Not enough data points for 2E optional.")
        return None
...
def plot_figure2e_optional(
    ...
    if output_dir is not None:
        ...
        for fig, name in zip(figs, names):
            ...
            plt.close(fig)
        print(f"Saved 2E optional to {out}")
